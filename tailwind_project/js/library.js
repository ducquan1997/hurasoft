...
function arccodionHandle(target, responsive) {
  ...
  $(target).on("click", function () {
    ...
    const $icon = $item.find(".arc-icon");
    const $content = $item.find(".arc-content");

    if (!$content.hasClass('arc-active')) {
      $container.find(".arc-content").slideUp().removeClass("arc-active");
      if ($icon) $container.find(".arc-icon").removeClass("arc-rotate");
    }

    if ($icon) $icon.toggleClass("arc-rotate");
    $content.slideToggle().toggleClass("arc-active");
  })
}
...
function searchHandle(target, type, tpl, limit, option) {
  ...
  function _searchPopup(value) {
    ...
    Hura.Ajax.get("search", params).then(function (respons) {
      _searchAsync(respons.list, respons.total, value);
    });
  }
...
function headerCartRender(data) {
  if (data.length === 0) return;

  let cart_total = 0, cart_quantity = 0;
  const cart_html = data.map(item => {
    cart_total += item.in_cart.total_price;
    cart_quantity += item.in_cart.quantity;

    const item_combo = item.item_type === "combo";
    const item_quantity = item.in_cart.quantity;
    const item_url = item_combo ? "javascript:;" : item.item_info.productUrl;
    const item_image = item_combo ? "LOGO" : item.item_info.productImage.large;
    const item_name = item_combo ? "Combo :" + item.item_info.title : item.item_info.productName;
    const item_variant = item.item_info.variant_info ? `<p class="compare-item-variant">Cấu hình: <span>${item.item_info.variant_info.name}</span></p>` : '';
    const item_price = item.in_cart.total_price ? formatCurrency(item.in_cart.total_price) + ' VNĐ' : 'Liên hệ';

    return `
      <div class="compare-item js-compare-item">
        <div class="compare-item-img">
          <a href="${item_url}" class="si-block">
            <img src="${item_image}" alt="${item_name}" class="si-content" />
          </a>
        </div>
        <div class="compare-item-info">
          ...
          <div class="compare-item-price">
            <b style="display: inline;color: #000;">x ${item_quantity}</b>
            <b style="width: auto;color: #000;">${item_price}</b>
          </div>
        </div>
      </div>
    `;
  ...
  $("#js-cart-ttip-container").html(cart_html);
  $("#js-cart-ttip-price-block").show();
  $("#js-header-cart-quantity").html(`<i>(Số lượng: ${cart_quantity} sản phẩm)</i>`);
  $("#js-header-cart-total-price").html(cart_total ? formatCurrency(cart_total) + ' VNĐ' : 'Liên hệ');
...
/**
 * Qiu form V5.0 - 12.12.2025
 */
/*
  CHECK FORM WITH INPUT/TEXTAREA HAS ATTRIBUTE [CHECK-TYPE]
  ...
  - SCRIPT:
    const checkForm = QiuForm('#form-id');
*/
