...
class BuildPC {
  ...
  selectItem(category_info, item_info, display_fn) {
    ...
    if (Array.isArray(this.select_config)) this.select_config = {};
    ...
    if (this.getItemIndexInCategory(category_info.id, item_info.id) == -1) {
      ...
      //show visually
      if (typeof display_fn === "function") display_fn();
    }
  }
  ...
  removeItem(category_id, item_id, display_fn) {
    ...
    if (item_index > -1) {
      ...
      //show visually
      if (typeof display_fn === "function") display_fn(category_id, item_id);
    }
  }

  //replace an item/product from list
  replaceItem(category_id, replaced_item_id, item_info, display_fn) {
    let item_index = this.getItemIndexInCategory(category_id, replaced_item_id);
    if (item_index > -1) {
      //replace item from array
      this.select_config[category_id].items.splice(item_index, 1, item_info);
      //show visually
      if (typeof display_fn === "function") display_fn(category_id, item_info);
    }
  }
